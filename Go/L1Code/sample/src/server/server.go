...
type Client struct {
	...
	digitCount [10]int       // Текущая сумма полученных от клиента дробей
	count      int
	sm         int
}
...
func (client *Client) handleRequest(req *proto.Request) bool {
	...
	case "add":
		...
		if req.Data == nil {
			...
			if err := json.Unmarshal(*req.Data, &number); err != nil {
				errorMsg = "malformed data field"
			} else {

				var digit int
				digit = number.Digit
				client.logger.Info("performing addition", "value", strconv.Itoa(digit))
				client.digitCount[digit]++
				client.count++
				client.sm = client.sm + digit
				//fmt.Print(client.sm)
			}
		}
		if errorMsg == "" {
	...
	case "count":
		fmt.Printf(strconv.Itoa(client.sm))
		errorMsg := ""
		if client.count == 0 {
			errorMsg = "there no number"
		}
		if req.Data == nil {
			errorMsg = "data field is absent"
		} else {
			...
			if err := json.Unmarshal(*req.Data, &number); err != nil {
				...
				//var digit int
				//digit = number.Digit
				client.respond("result", &proto.Number{
					Digit: client.sm,
				})
			}
		}
